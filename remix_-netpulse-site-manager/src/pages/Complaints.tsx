...
const Complaints: React.FC<{ profile: UserProfile | null }> = ({ profile }) => {
  ...
  const handleExport = () => {
    const dataToExport = filteredComplaints.map(c => ({
      ...
      'Latitude': c.lat,
      'Longitude': c.lng,
      'Nearest Sites': c.nearestSites?.map(ns => `${ns.siteId} (${ns.distance.toFixed(1)}km)`).join(', ') || ''
    }));
                      ...
                      {c.type === 'Network' && c.nearestSites && c.nearestSites.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1">
                          {c.nearestSites.map((ns, i) => (
                            <span key={i} className="px-1.5 py-0.5 rounded bg-amber-50 text-[7px] font-bold text-amber-700 border border-amber-100 flex items-center gap-1">
                              <span className="opacity-60">{ns.siteId}</span>
                              <span className="text-[6px] px-1 bg-amber-200/50 rounded-sm">{ns.distance.toFixed(1)}km</span>
                            </span>
                          ))}
                        </div>
                      )}
