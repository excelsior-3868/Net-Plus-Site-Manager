          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-6 text-ntc-blue/40">
              <div className="flex items-center gap-2 text-sm">
                <MapPin size={16} />
                <span>{site.admin?.wardToleArea ? `${site.admin.wardToleArea}, ` : ''}{site.admin?.district}, {site.admin?.zone}, {site.admin?.province}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                 <Shield size={16} />
                ...
                <section>
                   <h4 className="mb-4 flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-widest opacity-40 group/header">
                     ...
                     {canEdit && (
                       <button 
                         onClick={() => {
                           setInitialModalTab('infrastructure');
                           setModalOpen(true);
                         }}
                   ...
                   <div className="space-y-4 rounded-3xl border border-[#141414]/5 p-6">
                      <div className="grid grid-cols-2 gap-y-4">
                         ...
                         <div>
                            <p className="text-[10px] uppercase tracking-widest opacity-40">NT Fencing</p>
                            <p className="font-semibold">{site.tower?.fencingDoneByNT ? 'YES (DONE BY NT)' : 'NO'}</p>
                         </div>
                      </div>
                   ...
                   <h4 className="mb-4 flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-widest opacity-40 group/header">
                    ...
                    <div className="space-y-4 rounded-3xl border border-[#141414]/5 p-6">
                      <div className="grid grid-cols-2 gap-y-4">
                         ...
                         <div className="pt-2 border-t border-ntc-blue/5">
                            <p className="text-[10px] uppercase tracking-widest opacity-40">Supply Details</p>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1">
                                <div>
                                   <p className="text-[9px] uppercase tracking-wider opacity-60">Customer ID</p>
                                   <p className="text-xs font-mono font-bold">{site.power?.customerId || 'N/A'}</p>
                                </div>
                                <div>
                                   <p className="text-[9px] uppercase tracking-wider opacity-60">NT Transformer</p>
                                   <p className="text-xs font-bold">{site.power?.ntTransformer ? 'YES' : 'NO'}</p>
                                </div>
                                <div>
                                   <p className="text-[9px] uppercase tracking-wider opacity-60">Cable Type / Length</p>
                                   <p className="text-xs font-bold">{site.power?.powerCableType || 'N/A'} ({site.power?.powerCableLength || 0}m)</p>
                                </div>
                                <div>
                                   <p className="text-[9px] uppercase tracking-wider opacity-60">MCB Capacity</p>
                                   <p className="text-xs font-mono font-bold">{site.power?.mcbCapacity || 'N/A'}</p>
                                </div>
                             </div>
                          </div>
